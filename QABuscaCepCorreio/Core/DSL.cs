...
namespace QABuscaCepCorreio.Core
{
    public class DSL : GlobalVariables
    {
        ...
        {
            var espera = new WebDriverWait(driver, TimeSpan.FromSeconds(tempo));
        ...
        {
            var espera = new WebDriverWait(driver, TimeSpan.FromSeconds(60));
        ...
        public void EscreveTexto(string elemento, string valor, [Optional] string descricao)
        {
            ...
            {
                if (descricao != null)
                    Console.WriteLine("Erro ao preencher " + descricao + exceptionMsg + ex.Message);
        ...
        public void ClicaElemento(string elemento, [Optional] string descricao)
        {
            ...
            {
                if (descricao != null)
                    Console.WriteLine("Erro ao clicar no elemento " + descricao + exceptionMsg + ex.Message);
        ...
        public void ValidaDados(string elemento, string valor, [Optional] string descricao)
        {
            try
            {
                Assert.That(driver.FindElement(By.XPath(elemento)).Text, Does.Contain(valor));
            ...
            {
                if (descricao != null)
                    Console.WriteLine("Erro ao validar elemento" + descricao + exceptionMsg + ex.Message);
        ...
        public void MenuDropDown(string elemento, string valor, [Optional] string descricao)
        {
            try
            {
                ...
                driver.FindElement(By.XPath(elemento)).Click(); //encontra e clica no elemento de dropdown
                EsperaElemento(elementoValor); //espera as opções carregarem
                driver.FindElement(By.XPath(elementoValor)).Click(); //clica na opção especifica criada
                CliqueFora();
            ...
            {
                if (descricao != null)
                    Console.WriteLine("Erro ao selecionar opção no Dropdown: " + descricao + exceptionMsg + ex.Message);
